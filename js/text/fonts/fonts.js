...
import { composeBBoxes } from '/js/bbox.js'
...
class RenderedCharacter {
  ...
  get bbox() {
    if (this.contours.length == 0) {
      return new BBox(0, 0, 0, 0) // the ' ' can have no contours
    }
...
class Font {
  constructor({ name, shapes, hints, advances }) {
    this.name = name
    this.shapes = convertShapeMap(shapes)
    this.kerning = hints
  ...
  renderText(text, size) {
    ...
    for (let [c1, c2] of pairs([...text, ''])) {
      ...
      let charShapes = this.shapes[c1]
      let renderedCharacter = new RenderedCharacter(charShapes, c1, size, new Point(0, 0))
      ...
      xPosition += this.advances[c1]
      if (c1 + c2 in this.kerning) {
        xPosition += this.kerning[c1 + c2]
      }
    }
